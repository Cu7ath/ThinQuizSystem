...
public class AddQuestionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse res)
        ...
        try {
            ...
            if (row > 0) {
                out.println("<script>alert('Question added successfully'); window.location='"+ req.getContextPath() +"/addQuestion.jsp';</script>");
            } else {
                out.println("<script>alert('Failed to add question'); window.location='"+ req.getContextPath() +" /addQuestion.jsp';</script>");
            }
        } catch (Exception e) {
            out.println("<script>alert('Error: " + e.getMessage() + "'); window.location='"+ req.getContextPath() +"/addQuestion.jsp';</script>");
        }
    }
}
