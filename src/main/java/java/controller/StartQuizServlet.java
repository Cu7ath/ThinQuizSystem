    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (currentIndex >= questions.size()) {
                response.sendRedirect(request.getContextPath() +"/SubmitQuizServlet?quizId=" + quizId);
                return;
            }
            ...
            request.getRequestDispatcher(request.getContextPath() +"/startQuiz.jsp").forward(request, response);
        } catch (Exception e) {
            e.printStackTrace();
