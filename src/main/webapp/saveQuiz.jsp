...
<%
    ...
    try {
        ...
        conn.close();
        out.println("<script>alert('Quiz berjaya disimpan!'); location='"+ request.getContextPath() +"/ListQuiz.jsp';</script>");

    } catch (Exception e) {
        e.printStackTrace();
        out.println("<script>alert('Ralat semasa menyimpan kuiz: " + e.getMessage().replace("'", "\'") + "'); location='"+ request.getContextPath() +"/createQuiz.jsp';</script>");
    }
%>
