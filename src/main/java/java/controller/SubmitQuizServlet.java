...
public class SubmitQuizServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (!studentRs.next()) {
                out.println("<script>alert('Ralat pelajar.'); window.location='" + request.getContextPath() + "/studentHome.jsp';</script>");
                return;
            }
            int studentId = studentRs.getInt("id");

            // Insert ke student_submissions
            // Ambil submission_id dari form (hidden input)
            ...
            // Kemas kini total_score dan end time
            PreparedStatement updStmt = conn.prepareStatement("UPDATE student_submissions SET total_score=?, end_time=NOW(), evaluated=false WHERE id=?");
            updStmt.setInt(1, totalScore);
            updStmt.setInt(2, submissionId);
            updStmt.executeUpdate();

            conn.close();
            out.println("<script>alert('Quiz submitted successfully! Please provide your feedback.'); window.location='" + request.getContextPath() + "/feedbackForm.jsp?quiz_id=" + quizId + "';</script>");
        } catch (Exception e) {
            out.println("<script>alert('Ralat: " + e.getMessage() + "'); window.location='" + request.getContextPath() + "/studentHome.jsp';</script>");
        }
    }
}
